    ...
    methods: {
      ...
      rnInputChanged() {
        ...
        document.dispatchEvent(
          new CustomEvent("rnChanged", { detail: newDate })
        );
      },
      confirmRnInput() {
      ...
      editTimetable(timetable, date) {
        ...
        // add custom courses
        let customCourses =
          JSON.parse(localStorage.getItem("customCourses")) || [];
        customCourses.forEach((customCourse) => {
          ...
          if (
            customDay.getDate() == currentDay.getDate() &&
            customDay.getMonth() == currentDay.getMonth() &&
            customDay.getFullYear() == currentDay.getFullYear()
          ) {
            customCourse.course.time.start = st;
            customCourse.course.time.end = en;
            customCourse.course.course.color = subjectColor.getSubjectColor(
              customCourse.course.data.subject,
      ...
      handleRefresh(event) {
        ...
        this.$watch("timetable", () => {
          if (
            this.timetable.error != "STILL_LOADING" &&
            this.timetable.error != "ERR_BAD_REQUEST"
          ) {
            setTimeout(() => {
      ...
      openCoursModal(cours) {
        ...
        this.selectedCourse = {
          ...
          start: cours.time.start.toLocaleTimeString("fr-FR", {
            hour: "2-digit",
            minute: "2-digit",
          }),
          end: cours.time.end.toLocaleTimeString("fr-FR", {
            hour: "2-digit",
            minute: "2-digit",
          }),
      ...
      addNewCours() {
        ...
        if (
          stValue == "" ||
          this.$refs.newCoursEndRef.$el.value == "" ||
          this.$refs.newCoursNameRef.$el.value == "" ||
          this.$refs.newCoursTeacherRef.$el.value == "" ||
          this.$refs.newCoursRoomRef.$el.value == ""
        ) {
      ...
      deleteCustomCourse(id) {
        ...
        customCourses = customCourses.filter(
      ...
      swiper.on("slideChangeTransitionEnd", () => {
        setTimeout(() => {
          ...
          if (swiper.activeIndex == 0) {
            ...
            // emit event
            document.dispatchEvent(
              new CustomEvent("rnChanged", { detail: this.$rn })
            );
  ...
  <ion-page ref="page">
    <IonHeader class="AppHeader" collapse="fade" translucent>
      <IonToolbar>
        ...
        <ion-buttons slot="end">
          <ion-button
            mode="md"
            id="rnPickerModalButton"
            color="dark"
            @click="changernPickerModalOpen(true)"
          >
            <span class="material-symbols-outlined mdls" slot="start"
              >calendar_month</span
            >
    ...
    <ion-content :fullscreen="true">
      ...
      <IonFab
        ...
        vertical="bottom"
        horizontal="end"
        class="newCoursBtnFab"
      >
        <ion-button
          @click="setNewCoursModalOpen(true)"
          size="large"
          shape="round"
          class="newCoursBtn"
          mode="md"
        >
          <span class="material-symbols-outlined mdls" slot="icon-only"
            >add</span
          >
        </ion-button>
      </IonFab>
        ...
        <swiper-slide v-for="(day, i) in days" :key="i">
          <IonList>
            <CoursElement
              ...
              :start="
                cours.time.start.toLocaleString('fr-FR', {
                  hour: '2-digit',
                  minute: '2-digit',
                })
              "
              :end="
                cours.time.end.toLocaleString('fr-FR', {
                  hour: '2-digit',
                  minute: '2-digit',
                })
              "
            ...
            <div v-if="!$data[`${day}`].loading">
              <div v-if="!$data[`${day}`].error">
                <div class="NoCours" v-if="$data[`${day}`].length == 0">
                  <span class="material-symbols-outlined mdls">upcoming</span>
                  <h2>Pas de cours enregistrés pour cette journée</h2>
                  <p>
                    Réessayez un autre jour dans le calendrier ou balayez
                    l'écran.
                  </p>
                  ...
                  <ion-button
                    fill="clear"
                    @click="changernPickerModalOpen(true)"
                    class="changeDayButton"
                    >Ouvrir le calendrier</ion-button
                  >
                </div>
              </div>
            </div>
            ...
            <div v-if="!$data[`${day}`].loading">
              <div v-if="$data[`${day}`].error == 'ERR_NETWORK'" class="Error">
                <div class="NoCours" v-if="$data[`${day}`].length == 0">
                  <span class="material-symbols-outlined mdls">wifi_off</span>
                  <h2>Pas de connexion à Internet</h2>
                  <p>
                    Vous pouvez uniquement consulter les journées consultées à
                    l'avance lorsque vous êtes hors-ligne.
                  </p>
                </div>
              </div>
            </div>

            <div v-if="$data[`${day}`].loading" class="Error">
              <div class="NoCours" v-if="$data[`${day}`].length == 0">
                <IonSpinner></IonSpinner>
                <br />
                <h2>Téléchargement des prochains cours...</h2>
                <p>
                  Veuillez patienter pendant qu'on récupère vos cours depuis nos
                  serveurs...
                </p>
              </div>
            </div>
      ...
      <IonModal
        ref="newCoursModal"
        class="newCoursModal"
        :is-open="newCoursModalOpen"
        :presenting-element="presentingElement"
      >
        <IonHeader>
          <IonToolbar>
            <ion-buttons slot="start">
              <ion-button @click="setNewCoursModalOpen(false)"
                >Annuler</ion-button
              >
            </ion-buttons>
            <ion-title>Ajouter un cours</ion-title>
            <ion-buttons slot="end">
              <ion-button @click="addNewCours()" color="primary"
                >Ajouter</ion-button
              >
            </ion-buttons>
          </IonToolbar>
          ...
          <ion-list>
            <ion-item class="info-item">
              ...
              <ion-button
                ...
                class="itemBtn"
                slot="end"
                v-if="selectedCourse.custom"
                @click="deleteCustomCourse(selectedCourse.id)"
              >
                <span class="material-symbols-outlined mdls" slot="start"
                  >delete</span
                >
            ...
            <ion-item class="info-item">
              ...
              <ion-label>
                <p>Horaires</p>
                <h2>
                  De {{ selectedCourse.start }} à {{ selectedCourse.end }}
                </h2>
              </ion-label>
              <ion-button
                class="itemBtn"
                fill="clear"
                slot="end"
                @click="setNotif(selectedCourse.originalCourse)"
              >
                <span class="material-symbols-outlined mdls" slot="start"
                  >notifications</span
                >
            ...
            <ion-item
              class="info-item"
              v-if="selectedCourse.isCancelled"
            ...
            >
              ...
              <ion-label>
                <p>Statut</p>
                <h2>
                  Ce cours n'est pas maintenu<br />Motif :
            ...
            <ion-item
              class="info-item"
              v-else-if="selectedCourse.hasStatus"
