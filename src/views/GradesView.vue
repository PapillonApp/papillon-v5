...
export default defineComponent({
  ...
  data() {
    return {
      ...
      changePeriodSelection:
        localStorage.getItem("changePeriodSelection") == "true" ? true : false,
      selectedMark: {
        subject: "",
        average: 0,
        class_average: 0,
  ...
  methods: {
    ...
    editMarks(grades) {
      ...
      grades.forEach((subject) => {
        subject.marks.forEach((mark) => {
          ...
          if (out_of_20) {
            mark.grade.updated_value =
              (parseFloat(mark.grade.value) / parseFloat(mark.grade.out_of)) *
              20;
    ...
    setExcludedJoinSubject(subjectName) {
      ...
      localStorage.setItem(
        "excludedGroupSubjects",
        JSON.stringify(excludedGroupSubjects)
      );
      this.getGradesRefresh();
      this.$refs.averageModal.$el.dismiss();

      displayToast.presentToast(
        "Cette matière ne sera désormais plus regroupé.",
        "warning"
      );
    },
  ...
  mounted() {
    ...
    document.addEventListener("settingsUpdated", (ev) => {
      GetGrades().then((data) => {
        this.out_of_20 =
          localStorage.getItem("tweakGrades20") == "true" ? true : false;
        this.grades = this.editMarks(data.marks);
        this.fullGrades = this.editMarks(data.marks);
        this.averages = data.averages;
        this.isLoading = false;
...
<template>
  <ion-page ref="page">
    ...
    <ion-content :fullscreen="true">
      ...
      <ion-segment
        v-if="periods.length > 0 && changePeriodSelection"
        id="segment"
      ...
      >
        <div class="subject-name" @click="openAverageModal(subject)">
          <h3>
            {{ subject.name }}
            <span class="material-symbols-outlined mdls" v-if="subject.grouped"
              >join_inner</span
            >
          </h3>
          <p class="avg" v-if="subject.significant">
            {{ subject.average }}<small>/20</small>
          </p>
          <p class="avg" v-if="!subject.significant">
            {{ subject.significantReason }}<small>/20</small>
          </p>
        </div>
        ...
        <div class="grades">
          ...
          >
            <div class="myGrade">
              ...
              <p
                class="grd main"
                v-if="mark.info.significant && !mark.grade.updated_value"
              >
                {{ mark.grade.value }}<small>/{{ mark.grade.out_of }}</small>
              </p>
              <p
                class="grd main"
                v-else-if="mark.grade.updated_value && mark.info.significant"
              >
                {{ mark.grade.updated_value
                }}<small>/{{ mark.grade.updated_out_of }}</small>
              </p>
              <p
                class="coef"
                v-if="mark.grade.updated_value && mark.info.significant"
              >
                {{ mark.grade.value }}<small>/{{ mark.grade.out_of }}</small>
              </p>
              ...
              <p class="grd main" v-if="!mark.info.significant">
                {{ mark.info.significantReason
                }}<small>/{{ mark.grade.out_of }}</small>
              </p>
              <p
                class="coef"
                v-if="mark.grade.original_value && !mark.info.significant"
              >
                <br />
              </p>
            </div>
            <div class="averages" v-if="mark.info.significantAverage">
              <div class="average">
                <p class="grd">
                  {{ mark.grade.min }}<small>/{{ mark.grade.out_of }}</small>
                </p>
                <p>Min.</p>
              </div>
              ...
              <div class="average">
                <p class="grd">
                  {{ mark.grade.max }}<small>/{{ mark.grade.out_of }}</small>
                </p>
                <p>Max.</p>
              </div>
            </div>
...
.subject-name {
  ...
  align-items: center;
  padding: 10px 15px;
  background: linear-gradient(90deg, #00000055 0%, #00000055 100%),
    var(--backgroundTheme);
}
